...
_syncmlSection = sequence(
    ...
    p(_("""SyncML is an XML protocol designed to synchronize several
...
etc.""")),
    p(_("""%(name)s has built-in SyncML client support on Windows and Mac OS X
...
your Pocket PC.""") % meta.metaDict),
    p(_("""On Linux, you must install the SyncML client binding for
...
at the end of the Linux downloads section.""")),
    ...
    p(_("""This feature is optional and off by default. In order to turn it on,
...
should work."""),
    ...
    p(_("""Each task file has its own client ID, so that two different task 
...
<a target="_blank" href="%s"> Microsoft download site</a>.""") % _MSURL,
    ...
    p(_("""When SyncML is enabled, deleting a task or a note does not actually
...
problem, but takes a little more disk space.""")),
    p(_("""In this case, the "Purge deleted items" menu item in the File menu 
...
considered "soon".""")),
        ...
        p(_("""Before synchronizing, you must also configure %(name)s on the 
...
"iPhone" tab and fill in at least the password.""") % meta.metaDict),
        p(_("""When you tap the "Sync" button in the category view, %(name)s
...
be running."""
        ...
        p(_("""On Windows, you must install <a
...
(libavahi-compat-libdnssd1 on Ubuntu for instance)."""
        ...
        h5(_("""I can't seem to find the iPhone/iPod Touch app on Apple's 
website""")),
        p(_("""You need to have iTunes installed on your computer to browse 
...
format</a> and then edit your tasks on your Android device.""") % meta.metaDict
    ...
    p(_("""Todo.txt is an open source todo list manager, created by Gina 
...
file.""") % meta.metaDict),
    p(_("""When exporting to Todo.txt, %(name)s creates another file alongside
...
the Todo.txt file wins the conflict.""") % meta.metaDict),
    p(_("""Tip: if you save your task file in the todo folder that Todo.txt
...
while %(name)s only allows one parent task per task."""
    ...
    p(_("""When importing, %(name)s tries to find matching tasks and 
...
exported."""
    ...
    p(_("""%(name)s supports dates and times, but Todo.txt only supports 
dates, so the time part of start, due, and completion date/times is not
exported.""") % meta.metaDict),
    p(_("""The default Todo.txt format only supports planned start dates and 
...
todo.txt file.""") % meta.metaDict),
    p(_("""Todo.txt has priorities in the form of a letter ('A'-'Z'). 
