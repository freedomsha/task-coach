...
# On Windows, using wx.NewIdRef() in __Populate leads to errors after some time
...
class SmartDateTimeCtrl(wx.Panel):
    ...
    def SetRelativeChoicesStart(self, start=None):
        ...
        if self.__relButton is not None:
            self.__relButton.Enable(
                self.__timeCtrl.IsThisEnabled() and start is not None
            )
    ...
    def GetDateTime(self):
        if self.__dateCtrl.IsThisEnabled():
